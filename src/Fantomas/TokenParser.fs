...
let private isTokenAfterGreater token (greaterToken: Token) =
    ...
    greaterToken.TokenName = "GREATER"
...
let private hasOnlySpacesAndLineCommentsOnLine lineNumber tokens =
    if List.isEmpty tokens then
        ...
        |> List.forall (fun t ->
            t.TokenInfo.TokenName = "WHITESPACE"
            || t.TokenInfo.TokenName = "LINE_COMMENT")
...
let private isOperatorOrKeyword ({ TokenInfo = { CharClass = cc } }) =
    cc = FSharp.Compiler.SourceCodeServices.FSharpTokenCharKind.Keyword
    || cc = FSharp.Compiler.SourceCodeServices.FSharpTokenCharKind.Operator

let private isNumber ({ TokenInfo = tn }) =
    tn.ColorClass = FSharp.Compiler.SourceCodeServices.FSharpTokenColorKind.Number
...
let private findEmptyNewlinesInTokens (tokens: Token list) (lineCount) (ignoreRanges: FSharp.Compiler.Range.range list) =
    ...
    let completeEmptyLines =
        ...
        |> List.filter (fun line ->
            ...
            && not
                (List.exists (fun (br: FSharp.Compiler.Range.range) -> br.StartLine < line && br.EndLine > line)
                     ignoreRanges))
    ...
    let linesWithOnlySpaces =
        ...
        |> List.filter (fun (ln, g) ->
            ...
            <= lastLineWithContent
            && (List.length g) = 1
            && (List.head g).TokenInfo.TokenName = "WHITESPACE")
...
let getTriviaFromTokens (tokens: Token list) linesCount =
    ...
    let newLines =
        findEmptyNewlinesInTokens tokens linesCount (blockComments @ multilineStrings)
