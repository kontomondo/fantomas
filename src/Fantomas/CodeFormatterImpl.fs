    ...
    |> List.map (fun conditionalCompilationDefines ->
        async {
            let parsingOptionsWithDefines =
                { parsingOptions with
                      ConditionalCompilationDefines = conditionalCompilationDefines } // IsInteractive = false
...
let isValidAST ast =
    ...
    and validateMemberDefn (memberDefn: SynMemberDefn) =
        ...
        | SynMemberDefn.AutoProperty (_attributes, _isStatic, _id, _type, _memberKind, _memberFlags, _xmlDoc, _access,
                                      expr, _r1, _r2) -> validateExpr expr
    ...
    and validateBinding (Binding (_access, _bindingKind, _isInline, _isMutable, _attrs, _xmldoc, _valData, headPat,
                                  _retTy, expr, _bindingRange, _seqPoint))
        =
    ...
    and validateExpr =
        ...
        | SynExpr.TryWith (synExpr, _range, synMatchClauseList, _range2, _range3, _sequencePointInfoForTry,
        ...
        | SynExpr.LetOrUseBang (_sequencePointInfoForBinding, _, _, synPat, synExpr1, ands, synExpr2, _range) ->
            List.forall validateExpr [ synExpr1; synExpr2 ]
            && validatePattern synPat
            && List.forall (fun (pat, e) -> validateExpr e && validatePattern pat)
                   (ands
...
let formatWith ast defines formatContext config =
    ...
    let formattedSourceCode =
        let context =
            Fantomas.Context.Context.create config defines normalizedSourceCode (Some ast)

        context
        |> genParsedInput
            { ASTContext.Default with
                  TopLevelModuleName = moduleName } ast
...
let private formatRange
    ...
    let pre =
        if start = 0 then String.Empty else sourceCode.[0..start - 1].TrimEnd('\r')
    ...
    let post =
        if finish < sourceCode.Length then
            ...
            if String.startsWithOrdinal "\n" post then Environment.NewLine + post.[1..] else post
        else
            String.Empty
    ...
    async {
        ...
        | TypeMember ->
            ...
            if Array.isEmpty contents then
                if returnFormattedContentOnly
                then return result
                else return String.Join(String.Empty, pre, result, post)
            else
        ...
        | RecLet ->
            ...
            let pattern =
                if patch = RecType then Regex("type") else Regex("let rec")
...
let formatSelection
    ...
    (range: range)
    config
    ({ Source = sourceCode; FileName = fileName } as formatContext)
    =
    let lines =
    ...
    let contentRange =
        if range.StartLine = range.EndLine then
            ...
            let startCol =
                if startLine = range.StartLine then max range.StartColumn 0 else 0

            let endCol =
                if endLine = range.EndLine
                then min range.EndColumn (lines.[endLine - 1].Length - 1)
...
let readConfiguration fileOrFolder =
    try
        ...
        if List.isEmpty configurationFiles
        then failwithf "No configuration files were found for %s" fileOrFolder
