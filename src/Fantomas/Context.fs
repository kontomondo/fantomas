...
module WriterModel =
    ...
    let update maxPageWidth cmd m =
        ...
        let updateCmd cmd =
            ...
            | IndentBy x ->
                { m with
                      Indent = if m.AtColumn >= m.Indent + x then m.AtColumn + x else m.Indent + x }
            | UnIndentBy x ->
                { m with
        ...
        | ShortExpression info ->
            ...
            if info.IsTooLong maxPageWidth m.Column
               || nextCmdCausesMultiline then
                { m with
                      Mode = ShortExpression({ info with ConfirmedMultiline = true }) }
            else
                updateCmd cmd
...
type internal Context =
    ...
    member x.WithDummy(writerCommands, ?keepPageWidth) =
        ...
        let config =
            { x.Config with
                  PageWidth = if keepPageWidth then x.Config.PageWidth else Int32.MaxValue }
...
let internal writerEvent e ctx =
    ...
    let ctx' =
        { ctx with
              WriterEvents =
                  evs
                  |> Seq.fold (fun q x -> Queue.conj x q) ctx.WriterEvents
              WriterModel =
                  (ctx.WriterModel, evs)
                  ||> Seq.fold (fun m e -> WriterModel.update ctx.Config.PageWidth e m) }

    ctx'

let internal finalizeWriterModel (ctx: Context) =
    if ctx.WriterModel.WriteBeforeNewline <> ""
    then writerEvent (Write ctx.WriterModel.WriteBeforeNewline) ctx
    else ctx
...
let internal atIndentLevel alsoSetIndent level (f: Context -> Context) (ctx: Context) =
    if level < 0
    then invalidArg "level" "The indent level cannot be negative."
...
let internal (+>) (ctx: Context -> Context) (f: _ -> Context) x = f (ctx x)
...
let internal (+-) (ctx: Context -> Context) (str: string) x =
    ...
    let c =
        if c.BreakOn str then writerEvent WriteLine c else writerEvent (Write " ") c
...
let internal (+~) (ctx: Context -> Context) (str: string) x =
    ...
    let c =
        if addNewline c then writerEvent WriteLine c else c
...
let internal str (o: 'T) (ctx: Context) = ctx |> writerEvent (Write(o.ToString()))
...
let internal onlyIf cond f ctx = if cond then f ctx else id ctx

let internal onlyIfNot cond f ctx = if cond then id ctx else f ctx
...
let internal whenLastEventIsNotWriteLine f (ctx: Context) =
    if lastWriteEventIsNewline ctx then ctx else f ctx
...
let private shortExpressionWithFallback
    ...
    | ShortExpression _ when (maxWidth = ctx.Config.PageWidth) -> shortExpression ctx
    | _ ->
        ...
        | ShortExpression info ->
            ...
            if info.ConfirmedMultiline
               || info.IsTooLong ctx.Config.PageWidth resultContext.Column then
...
let internal leadingExpressionLong threshold leadingExpression continuationExpression (ctx: Context) =
    ...
    continuationExpression
        (lineCountAfter > lineCountBefore
         || (columnAfter - columnBefore > threshold)) contextAfterLeading
...
let private expressionExceedsPageWidth beforeShort afterShort beforeLong afterLong expr (ctx: Context) =
    ...
    | _ ->
        let shortExpressionContext =
            ctx.WithShortExpression(ctx.Config.PageWidth, 0)
...
let internal autoIndentAndNlnIfExpressionExceedsPageWidth expr (ctx: Context) =
    expressionExceedsPageWidth sepSpace sepNone  // before and after for short expressions
...
let internal autoNlnIfExpressionExceedsPageWidth expr (ctx: Context) =
    expressionExceedsPageWidth sepNone sepNone  // before and after for short expressions
...
let internal colAutoNlnSkip0i f' (c: seq<'T>) f (ctx: Context) =
    coli f' c (fun i c ->
        if i = 0
        then f i c
        else autoNlnIfExpressionExceedsPageWidth (f i c)) ctx
...
let internal sepColon (ctx: Context) =
    let defaultExpr =
        if ctx.Config.SpaceBeforeColon then str " : " else str ": "
...
let private removeNodeFromContext triviaNode (ctx: Context) =
    let newNodes =
        List.filter (fun tn -> tn <> triviaNode) ctx.Trivia
...
let internal printContentBefore triviaNode =
    ...
    +> removeBeforeContentOfTriviaNode
...
let private findTriviaMainNodeFromRange nodes (range: range) =
    ...
    |> List.tryFind (fun n ->
        Trivia.isMainNode n
        && RangeHelpers.rangeEq n.Range range)
...
let private findTriviaMainNodeOrTokenOnStartFromRange nodes (range: range) =
    ...
    |> List.tryFind (fun n ->
        ...
        && RangeHelpers.rangeEq n.Range range
        || Trivia.isToken n
           && RangeHelpers.rangeStartEq n.Range range)
...
let private findTriviaMainNodeOrTokenOnEndFromRange nodes (range: range) =
    ...
    |> List.tryFind (fun n ->
        ...
        && RangeHelpers.rangeEq n.Range range
        || Trivia.isToken n
           && RangeHelpers.rangeEndEq n.Range range)
...
let internal findTriviaTokenFromName (range: range) nodes (tokenName: string) =
    ...
    |> List.tryFind (fun n ->
        match n.Type with
        | Token (tn) when tn.TokenInfo.TokenName = tokenName -> RangeHelpers.``range contains`` range n.Range
        | _ -> false)
...
let internal enterNode (range: range) (ctx: Context) =
    enterNodeWith findTriviaMainNodeOrTokenOnStartFromRange range ctx

let internal enterNodeToken (range: range) (ctx: Context) =
    enterNodeWith findTriviaTokenFromRange range ctx
...
let internal leaveNodeWith f x (ctx: Context) =
    match f ctx.Trivia x with
    | Some triviaNode ->
        ((printContentAfter triviaNode)
         +> (removeNodeFromContext triviaNode)) ctx
    | None -> ctx

let internal leaveNode (range: range) (ctx: Context) =
    leaveNodeWith findTriviaMainNodeOrTokenOnEndFromRange range ctx

let internal leaveNodeToken (range: range) (ctx: Context) =
    leaveNodeWith findTriviaTokenFromRange range ctx
    ...
    |> fun tn ->
        ...
        | Some ({ ContentBefore = [ TriviaContent.Comment (LineCommentOnSingleLine (lineComment)) ] } as tn) ->
            let spacesBeforeComment =
                ...
                let spaceAround =
                    if ctx.Config.SpaceAroundDelimiter then 1 else 0

                !-String.Empty.PadLeft(braceSize + spaceAround)

            let spaceAfterNewline =
                if ctx.Config.SpaceAroundDelimiter then sepSpace else sepNone
...
let internal sepConsideringTriviaContentBefore sepF (range: range) ctx =
    ...
    | Some ({ ContentBefore = (Comment (BlockComment (_, false, _))) :: _ }) -> sepF ctx
    | Some ({ ContentBefore = contentBefore }) when (hasPrintableContent contentBefore) -> ctx
    | _ -> sepF ctx

let internal sepNlnConsideringTriviaContentBefore (range: range) =
    sepConsideringTriviaContentBefore sepNln range
...
let internal sepNlnConsideringTriviaContentBeforeWithAttributes (ownRange: range) (attributeRanges: range seq) ctx =
    ...
    |> fun hasContentBefore -> if hasContentBefore then ctx else sepNln ctx
...
let internal beforeElseKeyword (fullIfRange: range) (elseRange: range) (ctx: Context) =
    ...
    |> List.tryFind (fun tn ->
        match tn.Type with
        | Token (tok) ->
            tok.TokenInfo.TokenName = "ELSE"
            && (fullIfRange.StartLine < tn.Range.StartLine)
            && (tn.Range.StartLine >= elseRange.StartLine)
        | _ -> false)
    |> fun tn ->
        match tn with
        | Some ({ ContentBefore = [ TriviaContent.Comment (LineCommentOnSingleLine (lineComment)) ] } as tn) ->
...
let internal sepNlnBeforeMultilineConstruct range rangeOfAttributes ctx =
    ...
    if existingNewlines >= 2 then ctx // previous construct was multiline so no need to add any extra newlines
    else
