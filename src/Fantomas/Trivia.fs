...
let private findFirstNodeAfterLine (nodes: TriviaNode list) lineNumber: TriviaNode option =
    nodes
    |> List.filter (fun n ->
        n.Range.StartLine > lineNumber
        && isNotMemberKeyword n)
    |> fun filteredNodes ->
...
let private findNodeOnLineAndColumn (nodes: TriviaNode list) line column =
    nodes
    |> List.tryFindBack (fun { Range = range } ->
...
let private tryUpdateTriviaNode lens (triviaNodes: TriviaNode list) triviaNode =
    triviaNode
    |> Option.map (fun tn -> updateTriviaNode lens triviaNodes (Some tn))

let private findBindingThatStartsWith (triviaNodes: TriviaNode list) column line =
    triviaNodes
    |> List.tryFind (fun t ->
        match t.Type with
        | MainNode ("Binding") when (t.Range.StartColumn = column
                                     && t.Range.StartLine = line) -> true
        | MainNode ("SynPat.Named") when (t.Range.StartColumn = column
                                          && t.Range.StartLine = line) -> true
        | _ -> false)
...
let private findParsedHashOnLineAndEndswith triviaNodes startLine endColumn =
    ...
    |> List.tryFind (fun t ->
        match t.Type with
        | MainNode ("ParsedHashDirective") when (t.Range.StartLine = startLine
...
let private triviaBetweenAttributeAndParentBinding triviaNodes line =
    ...
    | Some (ai, a), Some (mdli, _) when (ai
                                         + 1 = mdli
                                         && a.Range.StartLine = a.Range.EndLine) -> Some a
    | _ -> None
...
let private addTriviaToTriviaNode (startOfSourceCode: int) (triviaNodes: TriviaNode list) trivia =
    ...
    | { Item = Comment (LineCommentOnSingleLine (_)) as comment; Range = range } when (commentIsAfterLastTriviaNode
        ...
        |> updateTriviaNode (fun tn ->
            { tn with
                  ContentAfter = List.appendItem tn.ContentAfter comment }) triviaNodes

    | { Item = Comment (LineCommentOnSingleLine (_) as comment); Range = range } ->
        findFirstNodeAfterLine triviaNodes range.StartLine
        |> updateTriviaNode (fun tn ->
            { tn with
                  ContentBefore = List.appendItem tn.ContentBefore (Comment(comment)) }) triviaNodes
        ...
        | (Some n), _ when n.Range.EndLine = range.StartLine ->
            ...
            |> updateTriviaNode (fun tn ->
                { tn with
                      ContentAfter =
                          tn.ContentAfter
                          @ [ Comment(BlockComment(comment, false, false)) ] }) triviaNodes
        | _, (Some n) ->
            Some n
            |> updateTriviaNode (fun tn ->
                ...
                { tn with
                      ContentBefore =
                          tn.ContentBefore
                          @ [ Comment(BlockComment(comment, false, newline)) ] }) triviaNodes
        | (Some n), _ ->
            Some n
            |> updateTriviaNode (fun tn ->
                { tn with
                      ContentAfter =
                          tn.ContentAfter
                          @ [ Comment(BlockComment(comment, true, false)) ] }) triviaNodes
    ...
    | { Item = Comment (LineCommentAfterSourceCode (_) as comment); Range = range } ->
        ...
        |> updateTriviaNode (fun tn ->
            { tn with
                  ContentAfter = List.appendItem tn.ContentAfter (Comment(comment)) }) triviaNodes
    ...
    | { Item = Newline; Range = range } when (range.StartLine > startOfSourceCode) ->
        ...
        | Some _ as node ->
            updateTriviaNode (fun tn ->
                { tn with
                      ContentAfter = List.appendItem tn.ContentAfter Newline }) triviaNodes node
        | _ ->
            let nodeAfterLine =
                findFirstNodeAfterLine triviaNodes range.StartLine
            ...
            | Some _ ->
                ...
                |> updateTriviaNode (fun tn ->
                    { tn with
                          ContentBefore = List.appendItem tn.ContentBefore Newline }) triviaNodes
            | None ->
                // try and find a node above
                findNodeBeforeLineFromStart triviaNodes range.StartLine
                |> updateTriviaNode (fun tn ->
                    { tn with
                          ContentAfter = List.appendItem tn.ContentAfter Newline }) triviaNodes

    | { Item = Keyword ({ Content = keyword } as kw); Range = range } when (keyword = "override"
                                                                            || keyword = "default"
                                                                            || keyword = "member") ->
        findMemberDefnMemberNodeOnLine triviaNodes range.StartLine
        |> updateTriviaNode (fun tn ->
            { tn with
                  ContentItself = Some(Keyword(kw)) }) triviaNodes

    | { Item = Keyword ({ TokenInfo = { TokenName = tn } } as kw); Range = range } when (tn = "QMARK") ->
        findConstNodeAfter triviaNodes range
        |> updateTriviaNode (fun tn ->
            { tn with
                  ContentBefore = List.appendItem tn.ContentBefore (Keyword(kw)) }) triviaNodes
    ...
    | { Item = Keyword ({ Content = keyword }); Range = range } when (keyword = "if"
        ...
        |> updateTriviaNode (fun tn ->
            { tn with
                  ContentItself = Some trivia.Item }) triviaNodes
    ...
    | { Item = Keyword (keyword); Range = range } ->
        ...
        |> fun nodeOnLineAndColumn ->
            ...
            | Some _ ->
                ...
                |> updateTriviaNode (fun tn ->
                    { tn with
                          ContentBefore = List.appendItem tn.ContentBefore (Keyword(keyword)) }) triviaNodes
            | None ->
    ...
    | { Item = Directive (dc) as directive; Range = range } ->
        ...
        | Some _ as node ->
            updateTriviaNode (fun tn ->
                { tn with
                      ContentAfter = List.appendItem tn.ContentAfter directive }) triviaNodes node
        | _ ->
            match findNodeAfterLineAndColumn triviaNodes range.EndLine range.EndColumn with
            | Some _ as node ->
                updateTriviaNode (fun tn ->
                    { tn with
                          ContentBefore = List.appendItem tn.ContentBefore directive }) triviaNodes node
            | None ->
                let findNode nodes =
                ...
                |> updateTriviaNode (fun tn ->
                    ...
                    { tn with
                          ContentAfter = List.appendItem tn.ContentAfter directive }) triviaNodes
...
let private triviaNodeIsNotEmpty triviaNode =
    not (List.isEmpty triviaNode.ContentAfter)
    || not (List.isEmpty triviaNode.ContentBefore)
