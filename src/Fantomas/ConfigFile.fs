...
let private allowedFileNames =
    [ jsonConfigFileName
      editorConfigFileName ]

let rec private getParentFolders acc current =
    let parent =
        Directory.GetParent(current) |> Option.ofObj
...
let rec findConfigurationFiles fileOrFolder: string list =
    ...
    if Directory.Exists fileOrFolder then
        ...
        | Some pf ->
            findConfigurationFiles pf.FullName
            @ [ fileOrFolder ]
        | None -> [ fileOrFolder ]
    else
        []

let makeWarningLocationAware configPath warning = sprintf "%s, in %s" warning configPath
...
let processSetting originalLine key value =
    ...
    | _ ->
        let warning =
            sprintf "%s is no valid setting for Fantomas v%s" originalLine (fantomasVersion.Value)
